#!/bin/bash
#
# $Id: replicate.sh 2.23 2016-01-25 18:43:03 rob.navarro $
#
...
if ! ssh $secondary "[[ -x $(which rsync) ]]" ; then
   echo "rsync must be installed on $secondary"
   exit 1
fi
...
# helper function to wrap up running, checking and possibly erroring a general shell command
function runcmd {
	local cmd="$*"
	if ! $cmd ; then
...
#
echo "  -- checking for ssh access issues" | tee -a $repl_log
